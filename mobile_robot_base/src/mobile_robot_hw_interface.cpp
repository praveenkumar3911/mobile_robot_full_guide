#include <mobile_robot_base/mobile_robot_hw_interface.h>
...
namespace mobile_robot_base
{
    mobile_robotHWInterface::mobile_robotHWInterface(ros::NodeHandle &nh, urdf::Model *urdf_model)
    ...
    { 
        ...
        // Get additional parameters from the mobile_robot_base/config/base.yaml which is stored on the parameter server
        ...
        pub_wheel_cmd_velocities_ = nh_.advertise<mobile_robot_msgs::WheelsCmdStamped>("wheel_cmd_velocities", 10);
        ...
        sub_encoder_ticks_ = nh_.subscribe("encoder_ticks", 10, &mobile_robotHWInterface::encoderTicksCallback, this);
        sub_measured_joint_states_ = nh_.subscribe("measured_joint_states", 10, &mobile_robotHWInterface::measuredJointStatesCallback, this);
    ...
    bool mobile_robotHWInterface::init(ros::NodeHandle &root_nh, ros::NodeHandle &robot_hw_nh)
    {
        ROS_INFO("Initializing mobile_robot Hardware Interface ...");
        ...
        ROS_INFO("... Done Initializing mobile_robot Hardware Interface");

        return true;
    }

    void mobile_robotHWInterface::read(const ros::Time& time, const ros::Duration& period)
    {
    ...
    void mobile_robotHWInterface::write(const ros::Time& time, const ros::Duration& period)
    {
        ros::Duration elapsed_time = period;
        // Write to robot hw
        ...
        mobile_robot_msgs::WheelsCmdStamped wheel_cmd_msg;
    ...
    bool mobile_robotHWInterface::isReceivingMeasuredJointStates(const ros::Duration &timeout)
    {
    ...
    void mobile_robotHWInterface::loadURDF(const ros::NodeHandle &nh, std::string param_name)
    {
    ...
    void mobile_robotHWInterface::printState()
    {
    ...
    std::string mobile_robotHWInterface::printStateHelper()
    {
        std::stringstream ss;
    ...
    std::string mobile_robotHWInterface::printCommandHelper()
    {
        std::stringstream ss;
        std::cout.precision(15);
    ...
    void mobile_robotHWInterface::encoderTicksCallback(const mobile_robot_msgs::EncodersStamped::ConstPtr& msg_encoder)
    {
    ...
    void mobile_robotHWInterface::measuredJointStatesCallback(const sensor_msgs::JointState::ConstPtr& msg_joint_states)
    {
        /// Update current encoder ticks in encoders array
        for (std::size_t i = 0; i < num_joints_; ++i)
        {
    ...
    double mobile_robotHWInterface::ticksToAngle(const int &ticks) const
    {
    ...
    double mobile_robotHWInterface::normalizeAngle(double &angle) const
    {
    ...
    double mobile_robotHWInterface::linearToAngular(const double &distance) const
    {
        return distance / wheel_diameter_ * 2.0;
    }

    double mobile_robotHWInterface::angularToLinear(const double &angle) const
    {
