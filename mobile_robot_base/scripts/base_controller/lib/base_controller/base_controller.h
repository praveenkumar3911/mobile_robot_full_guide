...
#ifndef mobile_robot_BASE_CONTROLLER_H
#define mobile_robot_BASE_CONTROLLER_H


#include <ros.h>
#include <mobile_robot_msgs/EncodersStamped.h>
#include <mobile_robot_msgs/WheelsCmdStamped.h>
#include <mobile_robot_msgs/AngularVelocities.h>
#include <mobile_robot_msgs/PIDStamped.h>
#include <std_msgs/Empty.h>
#include <sensor_msgs/JointState.h>

#include "mobile_robot_base_config.h"
#include "encoder_mobile_robot.h"
...
namespace mobile_robot {
     ...
     * 
     * The BaseController communicates with the high level interface mobile_robot_base::hardware_interface::RobotHW
     ...
     * (\ref mobile_robot_msgs::WheelsCmdStamped on the "wheel_cmd_velocities" topic) with the 
     ...
     * \ref mobile_robot::MotorControllerIntf. Each time a new \ref mobile_robot_msgs::WheelsCmdStamped is received on the
     ...
     * two \ref mobile_robot::Encoder objects (\ref encoder_left_ \ref encoder_right_)
     * are used to \ref read() the mobile_robot::JointStates, stored in \ref joint_state_left_ and \ref joint_state_left_.
     * After reading the latest states (position and velocity), the values are published with \ref pub_measured_joint_states_ on 
     * "measured_joint_states" topic of type \ref sensor_msgs::JointState. The \ref mobile_robot_base::hardware_interface::RobotHW
     ...
     * - /mobile_robot/encoder_resolution
     * - /mobile_robot/mobile_base_controller/wheel_radius
     * - /mobile_robot/mobile_base_controller/linear/x/max_velocity
     * - /mobile_robot/debug/base_controller
     * 
    ...
    public:
         ...
         * The only requirement is to implement the \ref mobile_robot::MotorControllerIntf,
         ...
         *   - /mobile_robot/mobile_base_controller/linear/x/max_velocity
         *   - /mobile_robot/debug/base_controller
         * - Initialize mobile_robot Wheel Encoders
         ...
         * \ref LastUpdateTime::command_received period on the /mobile_robot/wheel_cmd_velocities topic.
         * 
         */
        void eStop();
         ...
         * (e.g. mobile_robot::PID::error_ = \ref wheel_cmd_velocity_left_ - \ref measured_angular_velocity_left_)
         * 
         * The calculated PID ouput values are capped at -/+ MAX_RPM to prevent the PID from having too much error
         * The computed and capped PID values are then set using the \ref mobile_robot::MotorControllerIntf::setSpeed method
        ...
        /**
         * @brief Callback method when a new \ref mobile_robot_msgs::WheelsCmdStamped is received on the wheel_cmd_velocities topic.
         * 
         ...
         * 
         * This callback method receives \ref mobile_robot_msgs::WheelsCmdStamped message object (\p cmd_msg) consisting of
         * \ref mobile_robot_msgs::AngularVelocities, where commanded wheel joints for the left and right wheel are stored.
         ...
         * This is used for the eStop functionality. In case no mobile_robot_msgs::WheelsCmdStamped messages are 
         ...
         */
        void commandCallback(const mobile_robot_msgs::WheelsCmdStamped& cmd_msg);
         ...
         * Every time the mobile_robot_bringup/launch/bringup.launch is launched, the high level
         ...
         */
        void pidLeftCallback(const mobile_robot_msgs::PIDStamped& pid_msg);
         ...
         * @param pid_msg message containing the new PID constants
         */
        void pidRightCallback(const mobile_robot_msgs::PIDStamped& pid_msg);
    ...
    private:
        ...
        mobile_robot::Encoder encoder_left_;
        mobile_robot::Encoder encoder_right_;
        long ticks_left_ = 0, ticks_right_ = 0;

        // Measured left and right joint states (angular position (rad) and angular velocity (rad/s))
        mobile_robot::JointState joint_state_left_, joint_state_right_;
        ...
        mobile_robot_msgs::EncodersStamped encoder_msg_;
        ...
        ros::Subscriber<mobile_robot_msgs::WheelsCmdStamped, BaseController<TMotorController, TMotorDriver>> sub_wheel_cmd_velocities_;
        ...
        ros::Subscriber<mobile_robot_msgs::PIDStamped, BaseController<TMotorController, TMotorDriver>> sub_pid_left_;
        ros::Subscriber<mobile_robot_msgs::PIDStamped, BaseController<TMotorController, TMotorDriver>> sub_pid_right_;
        PID motor_pid_left_;
...
template <typename TMotorController, typename TMotorDriver>
using BC = mobile_robot::BaseController<TMotorController, TMotorDriver>;


template <typename TMotorController, typename TMotorDriver>
mobile_robot::BaseController<TMotorController, TMotorDriver>
    ::BaseController(ros::NodeHandle &nh, TMotorController* motor_controller_left, TMotorController* motor_controller_right)
    : nh_(nh)
...
template <typename TMotorController, typename TMotorDriver>
void mobile_robot::BaseController<TMotorController, TMotorDriver>::setup()
{
    nh_.initNode();
...
template <typename TMotorController, typename TMotorDriver>
void mobile_robot::BaseController<TMotorController, TMotorDriver>::init()
{
    nh_.loginfo("Get Parameters from Parameter Server");
    nh_.getParam("/mobile_robot/encoder_resolution", &this->encoder_resolution_);
    String log_msg = String("/mobile_robot/encoder_resolution: ") + String(encoder_resolution_);
    nh_.loginfo(log_msg.c_str());
    nh_.getParam("/mobile_robot/mobile_base_controller/wheel_radius", &wheel_radius_);
    log_msg = String("/mobile_robot/mobile_base_controller/wheel_radius: ") + String(wheel_radius_);
    nh_.loginfo(log_msg.c_str());
    nh_.getParam("/mobile_robot/mobile_base_controller/linear/x/max_velocity", &max_linear_velocity_);
    log_msg = String("/mobile_robot/mobile_base_controller/linear/x/max_velocity: ") + String(max_linear_velocity_);
    nh_.loginfo(log_msg.c_str());
    nh_.getParam("/mobile_robot/debug/base_controller", &debug_);
    log_msg = String("/mobile_robot/debug/base_controller: ") + String(debug_);
    nh_.loginfo(log_msg.c_str());

    nh_.loginfo("Initialize mobile_robot Wheel Encoders");
...
template <typename TMotorController, typename TMotorDriver>
void mobile_robot::BaseController<TMotorController, TMotorDriver>::commandCallback(const mobile_robot_msgs::WheelsCmdStamped& cmd_msg)
{
    ...
    // This callback function receives mobile_robot_msgs::WheelsCmdStamped message object
    // where mobile_robot_msgs::AngularVelocities for both joints are stored
    wheel_cmd_velocity_left_ = cmd_msg.wheels_cmd.angular_velocities.joint[0];
    wheel_cmd_velocity_right_ = cmd_msg.wheels_cmd.angular_velocities.joint[1];

    // Used for the eStop. In case no mobile_robot_msgs::WheelsCmdStamped messages are received on the wheel_cmd_velocities 
...
template <typename TMotorController, typename TMotorDriver>
void mobile_robot::BaseController<TMotorController, TMotorDriver>::resetEncodersCallback(const std_msgs::Empty& reset_msg)
{
...
template <typename TMotorController, typename TMotorDriver>
void mobile_robot::BaseController<TMotorController, TMotorDriver>::pidLeftCallback(const mobile_robot_msgs::PIDStamped& pid_msg)
{
    // Callback function to update the pid values of the left motor
    // This callback function receives mobile_robot_msgs::PID message object
    // where mobile_robot_msgs::PID kp, ki, kd for one pid controller is stored
    motor_pid_left_.updateConstants(pid_msg.pid.kp, pid_msg.pid.ki, pid_msg.pid.kd);
    motor_pid_left_.updateConstants(pid_msg.pid.kp, pid_msg.pid.ki, pid_msg.pid.kd);
}

template <typename TMotorController, typename TMotorDriver>
void mobile_robot::BaseController<TMotorController, TMotorDriver>::pidRightCallback(const mobile_robot_msgs::PIDStamped& pid_msg)
{
    // Callback function to update the pid values of the left motor
    // This callback function receives mobile_robot_msgs::PID message object
    // where mobile_robot_msgs::PID kp, ki, kd for one pid controller is stored
...
template <typename TMotorController, typename TMotorDriver>
void mobile_robot::BaseController<TMotorController, TMotorDriver>::read()
{
...
template <typename TMotorController, typename TMotorDriver>
void mobile_robot::BaseController<TMotorController, TMotorDriver>::write()
{
    // https://www.arduino.cc/reference/en/language/functions/math/map/
    // map(value, fromLow, fromHigh, toLow, toHigh)
    // Map angular wheel joint velocity to motor cmd
    // The /mobile_robot/mobile_base_controller/linear/x/max_velocity from the ROS parameter server 
...
template <typename TMotorController, typename TMotorDriver>
void mobile_robot::BaseController<TMotorController, TMotorDriver>::eStop()
{
...
template <typename TMotorController, typename TMotorDriver>
void mobile_robot::BaseController<TMotorController, TMotorDriver>::printDebug()
{
    /*
    String log_msg =
            String("\nRead:\n") +
...
#endif // mobile_robot_BASE_CONTROLLER_H