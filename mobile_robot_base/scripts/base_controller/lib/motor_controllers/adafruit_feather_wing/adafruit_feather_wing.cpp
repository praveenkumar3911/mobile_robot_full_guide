...
mobile_robot::AdafruitMotorController::AdafruitMotorController(uint8_t motor_num, uint8_t addr)
{
...
void mobile_robot::AdafruitMotorController::begin(uint16_t freq)
{
    motor_driver_.begin(freq);
}

void mobile_robot::AdafruitMotorController::setSpeed(int value)
{
    if (value > 0)
    {
        pMotor_->run(FORWARD);
